...
const hasNestedErrorCandidate = (value: unknown) => {
  ...
  const hasNestedEventTargetPayload =
    ...
    nestedTargetPayload.target?.cause !== undefined ||
    nestedTargetPayload.target?.reason !== undefined ||
    nestedTargetPayload.target?.errorReason !== undefined ||
    nestedTargetPayload.target?.message !== undefined ||
    ...
    nestedTargetPayload.target?.errorDetails !== undefined ||
    nestedTargetPayload.target?.info !== undefined ||
    nestedTargetPayload.target?.errorInfo !== undefined ||
    ...
    Array.isArray(nestedTargetPayload.target?.errors) ||
    Array.isArray(nestedTargetPayload.target?.causes) ||
    Array.isArray(nestedTargetPayload.target?.reasons) ||
    nestedTargetPayload.currentTarget?.error !== undefined ||
    nestedTargetPayload.currentTarget?.cause !== undefined ||
    nestedTargetPayload.currentTarget?.reason !== undefined ||
    nestedTargetPayload.currentTarget?.errorReason !== undefined ||
    nestedTargetPayload.currentTarget?.message !== undefined ||
    ...
    nestedTargetPayload.currentTarget?.errorDetails !== undefined ||
    nestedTargetPayload.currentTarget?.info !== undefined ||
    nestedTargetPayload.currentTarget?.errorInfo !== undefined ||
    ...
    Array.isArray(nestedTargetPayload.currentTarget?.errors) ||
    Array.isArray(nestedTargetPayload.currentTarget?.causes) ||
    Array.isArray(nestedTargetPayload.currentTarget?.reasons) ||
    nestedTargetPayload.srcElement?.error !== undefined ||
    nestedTargetPayload.srcElement?.cause !== undefined ||
    nestedTargetPayload.srcElement?.reason !== undefined ||
    nestedTargetPayload.srcElement?.errorReason !== undefined ||
    nestedTargetPayload.srcElement?.message !== undefined ||
    ...
    nestedTargetPayload.srcElement?.errorDetails !== undefined ||
    nestedTargetPayload.srcElement?.info !== undefined ||
    nestedTargetPayload.srcElement?.errorInfo !== undefined ||
    ...
    Array.isArray(nestedTargetPayload.srcElement?.errors) ||
    Array.isArray(nestedTargetPayload.srcElement?.causes) ||
    Array.isArray(nestedTargetPayload.srcElement?.reasons);
  ...
  return (
    ...
    ("errorReason" in value &&
      (value as { errorReason?: unknown }).errorReason !== undefined) ||
    ("errorMessage" in value &&
      (value as { errorMessage?: unknown }).errorMessage !== undefined) ||
    ...
    ("errorDetails" in value &&
      (value as { errorDetails?: unknown }).errorDetails !== undefined) ||
    ("info" in value && (value as { info?: unknown }).info !== undefined) ||
    ("errorInfo" in value &&
      (value as { errorInfo?: unknown }).errorInfo !== undefined) ||
    ("payload" in value &&
...
const getNextNestedError = (error: unknown) => {
  ...
  const nestedEventTargetError =
    ...
    nestedEventPayload.target?.cause ??
    nestedEventPayload.currentTarget?.error ??
    nestedEventPayload.currentTarget?.cause ??
    nestedEventPayload.srcElement?.error ??
    nestedEventPayload.srcElement?.cause ??
    nestedEventPayload.target?.reason ??
    nestedEventPayload.target?.errorReason ??
    nestedEventPayload.target?.message ??
    ...
    nestedEventPayload.target?.errorDetails ??
    nestedEventPayload.target?.info ??
    nestedEventPayload.target?.errorInfo ??
    nestedEventPayload.currentTarget?.reason ??
    nestedEventPayload.currentTarget?.errorReason ??
    ...
    nestedEventPayload.currentTarget?.errorDetails ??
    nestedEventPayload.currentTarget?.info ??
    nestedEventPayload.currentTarget?.errorInfo ??
    nestedEventPayload.srcElement?.reason ??
    nestedEventPayload.srcElement?.errorReason ??
    ...
    nestedEventPayload.srcElement?.errorDetails ??
    nestedEventPayload.srcElement?.info ??
    nestedEventPayload.srcElement?.errorInfo ??
  ...
  const nestedEventTargetArrayError =
    getFirstArrayEntry(nestedEventPayload.target?.errors) ??
    getFirstArrayEntry(nestedEventPayload.target?.causes) ??
    getFirstArrayEntry(nestedEventPayload.target?.reasons) ??
    getFirstArrayEntry(nestedEventPayload.currentTarget?.errors) ??
    getFirstArrayEntry(nestedEventPayload.currentTarget?.causes) ??
    getFirstArrayEntry(nestedEventPayload.currentTarget?.reasons) ??
    getFirstArrayEntry(nestedEventPayload.srcElement?.errors) ??
    getFirstArrayEntry(nestedEventPayload.srcElement?.causes) ??
    getFirstArrayEntry(nestedEventPayload.srcElement?.reasons);
  ...
  const errorReasonValue =
    "errorReason" in error
      ? getErrorPayloadOrSelf((error as { errorReason?: unknown }).errorReason)
      : undefined;
  const errorMessageValue =
    "errorMessage" in error
      ...
      : undefined;
  const msgValue =
    "msg" in error ? (error as { msg?: unknown }).msg : undefined;
  ...
  const errorInfoValue =
    "errorInfo" in error
      ? getErrorPayloadOrSelf((error as { errorInfo?: unknown }).errorInfo)
      : undefined;
  const payloadValue =
    "payload" in error
...
const getErrorMessage = (error: unknown) => {
  ...
  for (let depth = 0; depth < MAX_ERROR_CAUSE_TRAVERSAL_DEPTH; depth++) {
    ...
    if (
      "errorMessage" in currentError &&
      typeof currentError.errorMessage === "string"
    ) {
      const normalizedErrorMessageName = getErrorNameFromString(
        currentError.errorMessage.trim(),
      );
      if (!isGenericErrorName(normalizedErrorMessageName)) {
        return currentError.errorMessage;
      }
    }
    ...
    if (
      "errorReason" in currentError &&
      typeof currentError.errorReason === "string"
    ) {
      const normalizedErrorReasonName = getErrorNameFromString(
        currentError.errorReason.trim(),
      );
      if (!isGenericErrorName(normalizedErrorReasonName)) {
        return currentError.errorReason;
      }
    }
    if ("msg" in currentError && typeof currentError.msg === "string") {
      const normalizedMsgName = getErrorNameFromString(currentError.msg.trim());
      if (!isGenericErrorName(normalizedMsgName)) {
        return currentError.msg;
      }
    }
    if (
      "description" in currentError &&
      typeof currentError.description === "string"
    ) {
      const normalizedDescriptionName = getErrorNameFromString(
        currentError.description.trim(),
      );
    ...
    if (
      "errorDetails" in currentError &&
      typeof currentError.errorDetails === "string"
    ) {
      const normalizedErrorDetailsName = getErrorNameFromString(
        currentError.errorDetails.trim(),
      );
      if (!isGenericErrorName(normalizedErrorDetailsName)) {
        return currentError.errorDetails;
      }
    }
...
const getMediaRecorderRuntimeError = (event: unknown) => {
  ...
  const getEventLikeErrorPayload = (
    ...
    const nestedErrorReasonError =
      getObjectErrorPayload(nestedEventPayload.errorReason) ??
      getObjectErrorPayload(nestedEventPayload.target?.errorReason) ??
      getObjectErrorPayload(nestedEventPayload.currentTarget?.errorReason) ??
      getObjectErrorPayload(nestedEventPayload.srcElement?.errorReason);
    const nestedMessageError =
      getObjectErrorPayload(nestedEventPayload.message) ??
      getObjectErrorPayload(nestedEventPayload.target?.message) ??
      getObjectErrorPayload(nestedEventPayload.currentTarget?.message) ??
      getObjectErrorPayload(nestedEventPayload.srcElement?.message);
    const nestedErrorMessageError =
      getObjectErrorPayload(nestedEventPayload.errorMessage) ??
      getObjectErrorPayload(nestedEventPayload.target?.errorMessage) ??
      getObjectErrorPayload(nestedEventPayload.currentTarget?.errorMessage) ??
      getObjectErrorPayload(nestedEventPayload.srcElement?.errorMessage);
    const nestedMsgError =
      getObjectErrorPayload(nestedEventPayload.msg) ??
      getObjectErrorPayload(nestedEventPayload.target?.msg) ??
      getObjectErrorPayload(nestedEventPayload.currentTarget?.msg) ??
      getObjectErrorPayload(nestedEventPayload.srcElement?.msg);
    const nestedCauseError =
      getObjectErrorPayload(nestedEventPayload.cause) ??
      getObjectErrorPayload(nestedEventPayload.target?.cause) ??
      getObjectErrorPayload(nestedEventPayload.currentTarget?.cause) ??
      getObjectErrorPayload(nestedEventPayload.srcElement?.cause);
    ...
    const nestedDescriptionError =
      getObjectErrorPayload(nestedEventPayload.description) ??
    ...
    const nestedErrorDetailsError =
      getObjectErrorPayload(nestedEventPayload.errorDetails) ??
      getObjectErrorPayload(nestedEventPayload.target?.errorDetails) ??
      getObjectErrorPayload(nestedEventPayload.currentTarget?.errorDetails) ??
      getObjectErrorPayload(nestedEventPayload.srcElement?.errorDetails);
    const nestedInfoError =
      getObjectErrorPayload(nestedEventPayload.info) ??
      getObjectErrorPayload(nestedEventPayload.target?.info) ??
      getObjectErrorPayload(nestedEventPayload.currentTarget?.info) ??
      getObjectErrorPayload(nestedEventPayload.srcElement?.info);
    const nestedErrorInfoError =
      getObjectErrorPayload(nestedEventPayload.errorInfo) ??
      getObjectErrorPayload(nestedEventPayload.target?.errorInfo) ??
      getObjectErrorPayload(nestedEventPayload.currentTarget?.errorInfo) ??
      getObjectErrorPayload(nestedEventPayload.srcElement?.errorInfo);
    ...
    const nestedArrayError =
      getFirstArrayEntry(nestedEventPayload.errors) ??
      getFirstArrayEntry(nestedEventPayload.causes) ??
      getFirstArrayEntry(nestedEventPayload.reasons) ??
      getFirstArrayEntry(nestedEventPayload.target?.errors) ??
      getFirstArrayEntry(nestedEventPayload.target?.causes) ??
      getFirstArrayEntry(nestedEventPayload.target?.reasons) ??
      getFirstArrayEntry(nestedEventPayload.currentTarget?.errors) ??
    ...
    return getFirstArrayEntry([
      ...
      nestedCauseError,
      nestedReasonError,
      nestedErrorReasonError,
      nestedMessageError,
      nestedErrorMessageError,
      nestedMsgError,
      nestedTargetReasonError,
      nestedDescriptionError,
      nestedErrorDescriptionError,
      nestedErrorDetailsError,
      nestedInfoError,
      nestedErrorInfoError,
      nestedPayloadError,
      nestedDetailError,
      nestedDetailsError,
      nestedDataError,
      ...
      nestedEventPayload.currentTarget?.data,
      nestedEventPayload.srcElement?.data,
      nestedEventPayload.errors,
      nestedEventPayload.causes,
      nestedEventPayload.reasons,
      nestedEventPayload.target?.errors,
      nestedEventPayload.target?.causes,
      nestedEventPayload.target?.reasons,
      nestedEventPayload.currentTarget?.errors,
      nestedEventPayload.currentTarget?.causes,
      nestedEventPayload.currentTarget?.reasons,
      nestedEventPayload.srcElement?.errors,
      nestedEventPayload.srcElement?.causes,
      nestedEventPayload.srcElement?.reasons,
      nestedEventPayload.nativeEvent,
      nestedEventPayload.originalEvent,
    ]);
  };
  ...
  const getErrorReasonAliasPayload = (value: unknown) => {
    if (!value || typeof value !== "object" || !("error" in value)) {
      return undefined;
    }
    return (value as { error?: unknown }).error;
  ...
  const getMsgErrorPayload = (value: unknown) => {
    if (!value || typeof value !== "object" || !("error" in value)) {
      return undefined;
    }
    return (value as { error?: unknown }).error;
  ...
  const getMessageErrorPayload = (value: unknown) => {
    if (!value || typeof value !== "object" || !("error" in value)) {
      return undefined;
    }
    return (value as { error?: unknown }).error;
  ...
  const getErrorMessageAliasPayload = (value: unknown) => {
    if (!value || typeof value !== "object" || !("error" in value)) {
      return undefined;
    }
    return (value as { error?: unknown }).error;
  ...
  const getCauseErrorPayload = (value: unknown) => {
    if (!value || typeof value !== "object" || !("error" in value)) {
      return undefined;
    }
    return (value as { error?: unknown }).error;
  ...
  const getDescriptionErrorPayload = (value: unknown) => {
    if (!value || typeof value !== "object" || !("error" in value)) {
      return undefined;
    }
    return (value as { error?: unknown }).error;
  ...
  const getInfoErrorPayload = (value: unknown) => {
    if (!value || typeof value !== "object" || !("error" in value)) {
      return undefined;
    }
    return (value as { error?: unknown }).error;
  ...
  const getErrorDetailsPayload = (value: unknown) => {
    if (!value || typeof value !== "object" || !("error" in value)) {
      return undefined;
    }
    return (value as { error?: unknown }).error;
  ...
  const getErrorInfoPayload = (value: unknown) => {
    if (!value || typeof value !== "object" || !("error" in value)) {
      return undefined;
    }
    return (value as { error?: unknown }).error;
  ...
  const causeError =
    getCauseErrorPayload(eventPayload.cause) ??
    getCauseErrorPayload(eventPayload.target?.cause) ??
    getCauseErrorPayload(eventPayload.currentTarget?.cause) ??
    getCauseErrorPayload(eventPayload.srcElement?.cause);
  const reasonError =
  ...
  const errorReasonAliasError =
    getErrorReasonAliasPayload(eventPayload.errorReason) ??
    getErrorReasonAliasPayload(eventPayload.target?.errorReason) ??
    getErrorReasonAliasPayload(eventPayload.currentTarget?.errorReason) ??
    getErrorReasonAliasPayload(eventPayload.srcElement?.errorReason);
  const msgError =
    getMsgErrorPayload(eventPayload.msg) ??
    getMsgErrorPayload(eventPayload.target?.msg) ??
    getMsgErrorPayload(eventPayload.currentTarget?.msg) ??
    getMsgErrorPayload(eventPayload.srcElement?.msg);
  const messageError =
    getMessageErrorPayload(eventPayload.message) ??
    getMessageErrorPayload(eventPayload.target?.message) ??
    getMessageErrorPayload(eventPayload.currentTarget?.message) ??
    getMessageErrorPayload(eventPayload.srcElement?.message);
  const errorMessageAliasError =
    getErrorMessageAliasPayload(eventPayload.errorMessage) ??
    getErrorMessageAliasPayload(eventPayload.target?.errorMessage) ??
    getErrorMessageAliasPayload(eventPayload.currentTarget?.errorMessage) ??
    getErrorMessageAliasPayload(eventPayload.srcElement?.errorMessage);
  const descriptionError =
    getDescriptionErrorPayload(eventPayload.description) ??
  ...
  const errorDetailsError =
    getErrorDetailsPayload(eventPayload.errorDetails) ??
    getErrorDetailsPayload(eventPayload.target?.errorDetails) ??
    getErrorDetailsPayload(eventPayload.currentTarget?.errorDetails) ??
    getErrorDetailsPayload(eventPayload.srcElement?.errorDetails);
  const infoError =
    getInfoErrorPayload(eventPayload.info) ??
    getInfoErrorPayload(eventPayload.target?.info) ??
    getInfoErrorPayload(eventPayload.currentTarget?.info) ??
    getInfoErrorPayload(eventPayload.srcElement?.info);
  const errorInfoError =
    getErrorInfoPayload(eventPayload.errorInfo) ??
    getErrorInfoPayload(eventPayload.target?.errorInfo) ??
    getErrorInfoPayload(eventPayload.currentTarget?.errorInfo) ??
    getErrorInfoPayload(eventPayload.srcElement?.errorInfo);
  const payloadError =
  ...
  const arrayError =
    getFirstArrayEntry(eventPayload.errors) ??
    getFirstArrayEntry(eventPayload.causes) ??
    getFirstArrayEntry(eventPayload.reasons) ??
    getFirstArrayEntry(eventPayload.target?.errors) ??
    getFirstArrayEntry(eventPayload.target?.causes) ??
    getFirstArrayEntry(eventPayload.target?.reasons) ??
    getFirstArrayEntry(eventPayload.currentTarget?.errors) ??
    getFirstArrayEntry(eventPayload.currentTarget?.causes) ??
    getFirstArrayEntry(eventPayload.currentTarget?.reasons) ??
    getFirstArrayEntry(eventPayload.srcElement?.errors) ??
    getFirstArrayEntry(eventPayload.srcElement?.causes) ??
    getFirstArrayEntry(eventPayload.srcElement?.reasons);
  ...
  return (
    getFirstArrayEntry([
      ...
      eventPayload.target?.cause,
      eventPayload.currentTarget?.error,
      eventPayload.currentTarget?.cause,
      eventPayload.srcElement?.error,
      eventPayload.srcElement?.cause,
      causeError,
      reasonError,
      errorReasonAliasError,
      msgError,
      messageError,
      ...
      eventPayload.currentTarget?.errorDetails,
      eventPayload.currentTarget?.info,
      eventPayload.currentTarget?.errorInfo,
      eventPayload.srcElement?.reason,
      eventPayload.srcElement?.errorReason,
      ...
      eventPayload.srcElement?.errorDetails,
      eventPayload.srcElement?.info,
      eventPayload.srcElement?.errorInfo,
      ...
      originalEventError,
      eventPayload.reason,
      eventPayload.errorReason,
      eventPayload.cause,
      eventPayload.errorMessage,
      eventPayload.msg,
      eventPayload.description,
      eventPayload.errorDescription,
      eventPayload.errorDetails,
      eventPayload.info,
      eventPayload.errorInfo,

...
describe("useVideoRecorder", () => {
  ...
  it("falls back to MediaRecorder errorInfo.error when top-level fields are missing", async () => {
    const setup = setupRecordingFlowMocks();
    const recorder = renderUseVideoRecorder();
    ...
    act(() => {
      setup.emitRecorderErrorEvent({
        errorInfo: { error: { name: "NotSupportedError" } },
      });
    });
    await waitFor(() => {
      expect(recorder.latest.status).toBe("error");
  ...
  it("falls back to MediaRecorder target.errorInfo string payload when error fields are missing", async () => {
    const setup = setupRecordingFlowMocks();
    const recorder = renderUseVideoRecorder();
    ...
    act(() => {
      setup.emitRecorderErrorEvent({
        target: { errorInfo: "No such device" },
      });
    });
    await waitFor(() => {
      expect(recorder.latest.status).toBe("error");
  ...
  it("falls back to MediaRecorder errorDetails.error when top-level fields are missing", async () => {
    const setup = setupRecordingFlowMocks();
    const recorder = renderUseVideoRecorder();
    ...
    act(() => {
      setup.emitRecorderErrorEvent({
        errorDetails: { error: { name: "NotSupportedError" } },
      });
    });
    await waitFor(() => {
      expect(recorder.latest.status).toBe("error");
  ...
  it("falls back to MediaRecorder target.errorDetails string payload when error fields are missing", async () => {
    const setup = setupRecordingFlowMocks();
    const recorder = renderUseVideoRecorder();
    ...
    act(() => {
      setup.emitRecorderErrorEvent({
        target: { errorDetails: "No such device" },
      });
    });
    await waitFor(() => {
      expect(recorder.latest.status).toBe("error");

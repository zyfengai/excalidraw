...
describe("video recorder error mapping", () => {
  ...
  it("maps nested cause and reason payloads", () => {
    ...
    expect(
      mapVideoRecorderErrorMessage({
        underlyingError: { message: "No such device" },
      }),
    ).toBe("Selected camera or microphone is not available.");
    expect(
      mapVideoRecorderErrorMessage({
    ...
    expect(
      mapVideoRecorderErrorMessage({
        reason: "Error",
        payload: { error: { message: "No such device" } },
      }),
    ).toBe("Selected camera or microphone is not available.");
    expect(
      mapVideoRecorderErrorMessage({
    ...
    expect(
      mapVideoRecorderErrorMessage({
        name: "AggregateError",
        errors: [{ rootCause: { name: "NotSupportedError" } }],
      }),
    ).toBe("This browser does not support video recording.");
    expect(
      mapVideoRecorderErrorMessage({
    ...
    expect(
      mapVideoRecorderErrorMessage({
        target: { cause: { name: "NotSupportedError" } },
      }),
    ).toBe("This browser does not support video recording.");
    expect(
      mapVideoRecorderErrorMessage({
    ...
    expect(
      mapVideoRecorderErrorMessage({
        target: { message: { error: { name: "NotSupportedError" } } },
      }),
    ).toBe("This browser does not support video recording.");
    expect(
      mapVideoRecorderErrorMessage({
    ...
    expect(mapVideoRecorderErrorMessage(["Error", "No such device"])).toBe(
      "Selected camera or microphone is not available.",
    );
  });
  ...
  it("falls back to generic message", () => {
    ...
    expect(mapVideoRecorderErrorMessage(["Error"])).toBe(
      "Recording failed. Please try again.",
    );
    expect(
      mapVideoRecorderErrorMessage({
